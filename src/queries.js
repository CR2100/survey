...
app.get("/beans", function (req, res) {
  const sqlStatement =
    "INSERT INTO Login (username, password) VALUES ('ididit', 'right');";
...
app.post("/login", async (req, res) => {
  ...
  const user = connection.invokeQuery(
    sqlStatement,
    [username],
    async function (rows) {
      if (rows[0] !== undefined) {
...
app.post("/api/getLastSurvey", (req, res) => {
  var query =
    "SELECT Survey_ID FROM Survey WHERE Survey_ID = (SELECT max(Survey_ID) FROM Survey)";
...
app.put("/api/updateSurvey/:id", (req, res) => {
  ...
  query(
    "UPDATE Survey SET User_ID = ?, name = ?, survey_desc = ?, creation_date = NOW(), end_date = ? WHERE Survey_ID = ?",
    [uid, title, desc, expire, id],
    (err, result) => {
      if (err) {
        console.log(err);
      }
...
app.post("/api/updateQuestion/:id", (req, res) => {
  ...
  query(
    "UPDATE Questions SET Survey_ID = ?, Type_ID = ?, question_desc = ? WHERE Questions_ID = ?",
    [id, type, desc, qid],
    (err, result) => {
      if (err) {
        console.log(err);
...
app.post("/api/updateResponseOption/:id", (req, res) => {
  ...
  query(
    "UPDATE Response_Options SET Question_ID = ?, Survey_ID = ?, option = ? WHERE Option_ID = ?",
    [id, qid, option, oid],
    (err, result) => {
      if (err) {
        console.log(err);
...
app.post("/api/deleteResponseOption/:id", (req, res) => {
  ...
  query(
    "DELETE FROM Response_Options WHERE Option_ID = ?",
    [oid],
    (err, result) => {
      if (err) {
        console.log(err);
